...
args = getResolvedOptions(
    sys.argv, ["BUCKET_NAME", "TEMP_BUCKET_NAME", "DB_SECRET_NAME", "METADATA_FILEPATH", "SEMANTIC_FILEPATH", "QA_FILEPATH", "TIMESTAMP", "INVOKE_MODE"])
...
"""
def insert_analysis_results(meta_df, data_s, data_q):
...
@param data_q: dictionary
"""
def insert_analysis_results_new_guideline(data_s, data_q):
    
    all_syllabi_sql = "SELECT syllabus_id, s3_filepath FROM syllabus;"
    cursor.execute(all_syllabi_sql)
    syllabi = cursor.fetchall() # list of tuples
    syl_dict = [{"syllabus_id": s[0], "s3_filepath": s[1]} for s in syllabi] # list of dict
    ...
    for syl in syl_dict:
        ...
        for guideline_result in data_s.get(file_key).keys():
            ...
            analysis_result = {
                "syllabus_id": syl.get("syllabus_id"),
                "guideline_id": guideline_id,
            }
...
def main():
    ...
    # normal file upload invocation
    if INVOKE_MODE == "file_upload":
        meta_df = pd.read_csv(fetchFromS3(TEMP_BUCKET_NAME, METADATA_FILEPATH))
        insert_analysis_results(meta_df, data_s, data_q)
    # triggered by new guideline added on the front end
    elif INVOKE_MODE == "new_guideline":
        insert_analysis_results_new_guideline(data_s, data_q)
